...
import { PS, PSV, render, pose, pageSize, truthPoints, rnd, PITCH } from './synth.mjs';
...
function scene(name, cols, rows, w, h, s, rot, key, extra, slack, opts) {
  ...
  console.log(`${name}: finders ${res.finders.length}/${3 * cols * rows}, tiles ${idx.size}/${vis} visible (${cols * rows} drawn) in ${ms} ms` + (Object.keys(reasons).length ? ', rejected ' + JSON.stringify(reasons) : ''));
